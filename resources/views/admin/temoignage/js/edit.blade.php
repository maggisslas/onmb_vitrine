<script>
    ...
    function editTemoignage(element , pictureUrl){

        let temoignage  = JSON.parse(element.getAttribute("data-temoignage"));

        var route = "{{ route('admin.temoignages.update' , ':id') }}"
            route = route.replace(':id' , temoignage.id)

        preloadFile("pictureUpdate" , "pictureListUpdate" , pictureUrl)

        $('#updateModal form').attr('action' , route)
        $('#updateModal form #lastname').val(temoignage.lastname)
        $('#updateModal form #firstname').val(temoignage.firstname)
        $('#updateModal form #star').val(temoignage.star).trigger('change')
        $('#updateModal form #type').val(temoignage.type).trigger('change')
        $('#updateModal form #comment').val(temoignage.comment)

    }


    $('#temoignage-update-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $('#temoignage-update-form')[0].reset(); // Réinitialiser le formulaire
